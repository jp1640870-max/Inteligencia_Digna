...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[#030812] text-white flex items-center justify-center">
            ...
            {editingName ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="text-3xl font-bold bg-[#121824] border border-green-500 rounded-xl px-3 py-2 outline-none w-full max-w-md"
                  autoFocus
                />
              </div>
            ) : (
              ...
              {project.chats.map((chat) => (
                <div key={chat.id} className="group flex items-center gap-2">
                  <a
                    ...
                    className="flex-1 flex items-center gap-3 bg-[#121824] border border-[#202938] hover:border-green-500/50 rounded-xl px-4 py-3 transition-colors"
                  >
      ...
      {showAddChatModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowAddChatModal(false)}>
          <div className="bg-[#121824] border border-[#202938] rounded-2xl p-6 w-96 max-h-[70vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold mb-4">Agregar chat existente</h3>
